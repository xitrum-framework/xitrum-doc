...
(위의 mystyle.css 와 mystyle.min.css):
...
만약 요청된 URL이 pathRegex 와 맞지 않으면 Xitrum은 해당 요청에 404를 응답합니다
...
예를들어 `Underscore.js <http://underscorejs.org/>`_ 를 사용하고자 하는 경우,
...
Xitrum은 자동으로 `Etag <http://ja.wikipedia.org/wiki/HTTP_ETag>`_ 을 디스크내 클래스 패스의 정적파일을 사용하기 위해 추가합니다.
...
큰 파일의 경우, 수정된 시간만을 ETag에 사용됩니다. 완벽하지는 않지만 각기 서버는 다른 ETag 정보를 가질것으로 예상되기 때문입니다.
...
Xitrum은 헤더의 ``max-age`` 와 ``Expires`` 를 `1년 <https://developers.google.com/speed/docs/best-practices/caching>`_ 으로 설정합니다.
