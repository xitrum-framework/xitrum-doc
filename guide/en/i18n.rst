...
* To get languages set in the ``Accept-Language`` request header by the browser,
  call ``browserLanguageRanges``. The result is sorted by priority set by the brower,
  from high to low.
* The default language is "en". To set language for the current action, for example
  Japanese, call ``locale = java.util.Locale.JAPANESE``.
* To autoset the most suitable language in resources, call
  ``autosetLocale(availableLocales)``, where
  ``availableLocales`` is a list of locales corresponding to .po files in
  ``resources/i18n`` directory and JAR files.
  If there's no suitable language, the locale is still the default "en".
* To get the current locale set above, use ``locale``.
